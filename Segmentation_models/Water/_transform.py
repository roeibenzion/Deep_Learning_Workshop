...
def get_transforms(level='basic', dim=320):
    common_transforms = [
    ...
    if level == 'extreme':
        common_transforms += [
            ...
            A.RandomCrop(height=dim, width=dim, p=0.5),
            A.RandomScale(scale_limit=0.2, p=0.5),
        ]

    def apply_transforms(img, mask=None):
        augmented = A.Compose(common_transforms)(image=np.array(img), mask=np.array(mask))
        return Image.fromarray(augmented["image"]), Image.fromarray(augmented["mask"])

    
    image_transforms = transforms.Compose([
        lambda img, mask: apply_transforms(img, mask),
    ...
    return image_transforms, mask_transforms, val_image_transform, val_mask_transform
