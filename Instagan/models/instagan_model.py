...
class InstaGANModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        self.netG_A = networks.define_G(opt.input_nc, opt.output_nc, opt.ngf, opt.netG, opt.norm, not opt.no_dropout, opt.init_type, opt.init_gain, self.gpu_ids,use_attention=opt.use_attention)
        self.netG_B = networks.define_G(opt.output_nc, opt.input_nc, opt.ngf, opt.netG, opt.norm, not opt.no_dropout, opt.init_type, opt.init_gain, self.gpu_ids,use_attention=opt.use_attention)
        if self.isTrain:
        ...
        if self.isTrain:
            if len(self.gpu_ids) < -1:
                print('pp1')
                self.netG_A = nn.DataParallel(self.netG_A, device_ids=self.gpu_ids)
                self.netG_B = nn.DataParallel(self.netG_B, device_ids=self.gpu_ids)
                self.netD_A = nn.DataParallel(self.netD_A, device_ids=self.gpu_ids)
                self.netD_B = nn.DataParallel(self.netD_B, device_ids=self.gpu_ids)
    ...
    def select_masks_decreasing(self, segs_batch):
        ...
        #   ret.append(segs[i, :, :])
        return segs_batch#torch.stack(ret)
